...
class Survey(BaseModel):
    ...
    slug = models.SlugField(_("slug"), max_length=900, default='')
    ...
    answered_by = models.ManyToManyField(get_user_model(), blank=True, related_name="answered_surveys", verbose_name=_("answered by"))
...
class Question(BaseModel):
    ...
    key = models.CharField(_("key"), max_length=900, unique=True, null=True, blank=True,
    ...
    label = models.TextField(_("label"), help_text=_("Enter your question in here."))
...
class UserAnswer(BaseModel):
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        self.survey.answered_by.add(self.user)
...
class Answer(BaseModel):
    ...
    class Meta:
        ...
        unique_together = ('question', 'user_answer')  # Add a unique constraint to prevent duplicate answers
...
@receiver(m2m_changed, sender=Survey.answered_by.through)
def update_answered_by(sender, instance, **kwargs):
    if "post_add" in kwargs["action"]:
        instance.answered_by.add(*kwargs["pk_set"])
    elif "post_remove" in kwargs["action"]:
        instance.answered_by.remove(*kwargs["pk_set"])